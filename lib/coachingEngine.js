 ...
 * May 19, 2026 (Phase 8A DISC dual-output in detectPersona).
 *
 * Public API:
 *
...
async function detectPersona({ conversationId, force = false } = {}) {
  try {
    ...
    // Propagate to leads if linked. Phase 8A: also propagates DISC fields.
    // Leads always show the MOST RECENT call's classification — if a customer
    // calls back and we re-classify, the lead reflects the latest read.
    if (conv.lead_id) {
      await db.query(
        `UPDATE leads
            SET buyer_persona = $1,
...
module.exports = {
  ...
  validateAndNormalizeDisc, // exported for tests
  SCORING_DIMENSIONS,
  PERSONAS,
  DISC_QUADRANTS,
};
