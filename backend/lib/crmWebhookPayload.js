...
function normalizePhoneForCrm(raw) {
  ...
  if (!rawTrim) return "";
  // Facebook / website thread keys are not dialable numbers — pass through unchanged.
  if (/^fb-/i.test(rawTrim) || /^web-/i.test(rawTrim)) return rawTrim;

  let s = rawTrim.replace(/[\s().-]/g, "");
  if (!s) return "";
  if (s.startsWith("+")) return s.length >= 8 ? s : "";
  if (/^1\d{10}$/.test(s)) return `+${s}`;
  if (/^\d{10}$/.test(s)) return `+1${s}`;
  if (s.startsWith("00")) return `+${s.replace(/^00+/, "")}`;
