...
async function createCheckoutSession(tenantId, planId, returnUrl) {
    ...
    if (tenant?.business_type === "location") {
        ...
        targetSetupFeeId = process.env.STRIPE_SETUP_BASIC || "price_1T9PL4G4V3F53niETsVTW4hG";
...
async function createAddonNumberCheckoutSession(tenantId, returnUrl) {
    ...
    const session = await stripe.checkout.sessions.create({
        ...
        line_items: [
            {
                price: process.env.STRIPE_PRICE_ADDON_NUMBER || "price_1THM11G4V3F53niEsbKk3ruQ", // $12/mo add-on
                quantity: 1,
            },
        ],
        metadata: {
