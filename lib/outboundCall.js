...
const callOpts = {
    ...
    method: "GET",
    timeout,
    // Phase 8B visibility V2 (May 28, 2026): record every outbound AI call.
    // Twilio captures both legs; the recording's twilio_sid arrives via the
    // status callback. Future PR wires the status callback to INSERT into
    // the `recordings` table so the dashboard's CallDetail audio player
    // picks it up — for now, this just ensures the audio exists in Twilio's
    // storage so it's available when we wire the bridge.
    record: true,
  ...
  if (statusCallback) {
    ...
    // The same status callback URL also gets recording-completed events
    // since we set recordingStatusCallbackEvent above. The handler in
    // routes/twilio.js will branch on whether RecordingSid is present.
    callOpts.recordingStatusCallback = statusCallback;
    callOpts.recordingStatusCallbackMethod = "POST";
  }
