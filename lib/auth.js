...
function getTenantIdFromQuery(req) {
  ...
  // Apr 29, 2026 — superadmin impersonation. When a superadmin impersonates
  // a tenant via the admin console, the frontend sends `x-impersonate-tenant-id`
  // on every request. This is treated as the highest-priority tenant context,
  // overriding the user's own tenant_id.
  //
  // Security: only honored when req.user.is_super_admin === true. Non-superadmins
  // cannot escape their tenant scope by setting this header.
