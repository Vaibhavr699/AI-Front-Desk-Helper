...
function canRunOutboundFollowup(tenant) {
  ...
  if (typeof addons.outbound_followup === "boolean") {
    return addons.outbound_followup;
  }
...
function canRunOutboundLists(tenant) {
  if (!tenant) return false;
  const addons = tenant.plan_overrides?.addons || {};
  if (typeof addons.outbound_lists === "boolean") {
    return addons.outbound_lists;
  }
...
function getOutboundDailyMax(tenant) {
  if (!tenant) return 0;
  const addons = tenant.plan_overrides?.addons || {};
  if (typeof addons.outbound_daily_max === "number") {
    return addons.outbound_daily_max;
  }
