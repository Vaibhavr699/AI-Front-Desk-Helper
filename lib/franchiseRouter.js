...
// ── Context resolution ────────────────────────────────────────────────────────
/**
 * Resolve franchisor context directly from a known parent tenant id, skipping
 * the phone→tenant lookup. Used by the WS handler, where Twilio has stripped
 * the dialed number from the query string but the parent id is in the path.
 * Same master-gate + children requirements and same return shape as
 * resolveInboundContext. Never throws — returns { mode: "direct" } on any
 * failure or if the gate/children checks don't pass.
 */
async function resolveByParentId(parentId) {
  try {
    if (!parentId) return { mode: "direct" };
    ...
    if (children.length === 0) return { mode: "direct" };

    return { mode: "franchisor", parent, children };
  } catch (err) {
    console.error("[FranchiseRouter] resolveByParentId failed parentId=%s err=%s — defaulting to direct", parentId, err.message);
    return { mode: "direct" };
  }
}
...
async function resolveInboundContext(toNumber) {
  try {
    ...
    //
    // Match the SAME normalized way lib/tenant.js getTenantByPhone does —
    // last-10-digits compare — NOT an exact-string match. Twilio sends
    // "+14025551234" while stored numbers may be formatted differently; an
    // exact match would silently miss and make a real franchisor number
    // resolve as "direct". Keep this in lockstep with getTenantByPhone.
    const last10 = String(toNumber).replace(/\D/g, "").slice(-10);
    if (last10.length < 10) return { mode: "direct" };
