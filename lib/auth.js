...
/**
 * Ensures we return a valid UUID for single-tenant routes.
 * If 'all' is requested, it falls back to the user's primary tenant.
 */
 ...
 *  Supports roll-up for parents. Returns an array of UUIDs.
 */
async function getTargetTenantIds(req) {
  ...
  // 1. Parent Business: Expand 'all' or show HQ rollup if requested
  ...
  // 2. Super Admin: If 'all' is requested, resolve to everything
  if (req.user?.is_super_admin && tenantId === 'all') {
    const r = await db.query("SELECT id FROM tenants ORDER BY name ASC");
    return r.rows.map(row => row.id);
  }

  // 3. Safety: If 'all' reached here but user isn't authorized for rollup, 
  // revert to their assigned tenant to prevent SQL errors.
  if (tenantId === 'all') {
    return req.user?.tenant_id ? [req.user.tenant_id] : [];
  }

  // 4. Default: Return the single resolved ID
  return [tenantId];
}
