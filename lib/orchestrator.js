...
const REALTIME_TOOLS = [
  ...
  // funnel.
  //
  // Phase E1.1 (May 4, 2026, later same day): also exposed in recovery
  // flow via RECOVERY_TOOLS filter. Use case: customer on a recovery
  // callback says "what was that price again?" or "we added a room" —
  // AI sends a fresh link instead of trying to quote verbally.
  // ─────────────────────────────────────────────────────────────────────
  {
    type: "function",
...
//
// Phase E1.1 (May 4, 2026): added send_estimate_link to the filter so
// recovery callbacks can also offer to text a fresh estimate link.
// Common recovery scenarios: customer asks "what was that price?" or
// "we added a room", or has lost the original SMS. Routing through the
// estimator avoids the AI hallucinating a price verbally.
// ─────────────────────────────────────────────────────────────────────
...
//
// Phase E1.1 (May 4, 2026, later same day): rules now apply to BOTH
// inbound and recovery flows. The recovery flow gets the tool too so
// callbacks can offer to text a fresh link when the customer asks
// "what was that price?" or wants to revise scope. Wording uses
// "customer" rather than "caller" so it reads naturally in the
// outbound recovery context (where the AI is technically the caller).
// ═════════════════════════════════════════════════════════════════════
...
function buildInstructions(context) {
  ...
  // Estimate link rules — applied to BOTH inbound AND recovery flows
  // (Phase E1.1, May 4, 2026). send_estimate_link is now in both tool
  // lists, so the AI in either context knows how to use it. Wording in
  // ESTIMATE_LINK_RULES is generic enough for both contexts.
