...
//   NOT a labor multiplier — labor cost variance baked into per-square rates
//   for V1.
// ============================================================================
...
async function calculateRange(tenantId, serviceSlug, inputs = {}) {
  ...
  if (serviceSlug === "interior") {
    ...
    // All 5 roof types share the same per-square math; serviceSlug picks the rate row.
    ({ baseMin, baseMax } = await computeRoofingBase(tenant.vertical_id, bucketRes.bucket, serviceSlug, inputs));
  } else {
...
//      (1-story home: total ≈ footprint. 2-story: total / 2 = footprint.)
//
...
//   low    → 1.0308 (reference 3/12 pitch, low slope)
//   medium → 1.1180 (reference 6/12 pitch, most common residential)
//   steep  → 1.3017 (reference 10/12 pitch, cathedral/victorian)
//   (very_steep and three_plus stories are filtered upstream by
//    checkSpecialized — they never reach this function. Defensive throws
//    below cover widget-config drift.)
//
// Labor difficulty for steep pitches and multi-story homes is currently
// baked into the per-square mid-bucket rates (V1 simplification). Once
// real contractor pricing data is available in V1.1, separate labor
// modifiers can be added as vertical_modifier_categories rows.
// ----------------------------------------------------------------------------
async function computeRoofingBase(verticalId, bucket, serviceSlug, inputs) {
  // Mighty Dog Roofing pitch calculator midpoints — pure Pythagorean geometry.
  const pitchMultiplier = {
    low:    1.0308,  // ~3/12 — nearly flat, modern ranch, additions
  ...
  // Stories is a footprint DIVISOR, not a labor multiplier.
  // A 2000 sqft 2-story home has ~1000 sqft footprint, ~1000 sqft roof footprint.
