...
async function book({
  ...
  if (finalLeadId) {
    ...
    // Back-fill the parent lead's project_type + estimated_revenue_cents from
    // this booking, but ONLY where the lead's fields are still empty — never
    // overwrite a value the lead already carries (a repeat customer may have
    // a prior project_type/revenue we don't want to clobber). Fixes the gap
    // where a voice/SMS booking wrote scope + estimated value to the booking
    // row but left the lead at project_type=NULL / estimated_revenue_cents=0,
    // starving the dashboard + flywheel of the lead-level signal. The booking
    // already holds the canonical values via createBooking/normalizeBookingData;
    // we mirror them up to the lead here. Fire-and-forget — never block or
    // fail the booking on this enrichment.
    const enrichType = (projectType && String(projectType).trim()) || null;
    const enrichCents = (Number.isFinite(numericEstimate) && numericEstimate > 0)
      ? Math.round(numericEstimate * 100)
      : null;
    if (enrichType || enrichCents) {
      db.query(
        `UPDATE leads
            SET project_type = COALESCE(NULLIF(project_type, ''), $2),
                estimated_revenue_cents = CASE
                  WHEN COALESCE(estimated_revenue_cents, 0) = 0 THEN COALESCE($3, estimated_revenue_cents)
                  ELSE estimated_revenue_cents
                END,
                updated_at = now()
          WHERE id = $1`,
        [finalLeadId, enrichType, enrichCents]
      ).catch((e) =>
        console.error("[bookingEngine.book] lead enrichment failed leadId=%s: %s", finalLeadId, e.message)
      );
    }
  }
