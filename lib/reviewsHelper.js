...
async function handleOAuthCallback(tenantId, code) {
  ...
  try {
    ...
    console.error(
      "[Reviews] accounts.list failed status=%s data=%s msg=%s",
      err.response?.status,
  ...
  if (accountName) {
    try {
      ...
      console.error(
        "[Reviews] locations.list failed status=%s data=%s msg=%s",
        err.response?.status,
        JSON.stringify(err.response?.data || {}),
        err.message
      );
...
async function fetchGoogleReviews(tenant) {
  ...
  return reviews.map((r) => ({
    ...
    hasOwnerReply: !!r.reviewReply,   // true if the business already replied
    raw:           r,
  }));
}
...
async function generateAIDraft(tenant, review) {
  ...
  // Build a natural local-area descriptor for light SEO (e.g. "Omaha, NE").
  // Pulled from the tenant's own city/state — the business's service area,
  // NOT the customer's address. Used for ONE natural mention, never stuffed.
