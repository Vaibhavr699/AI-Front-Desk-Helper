...
async function handleResellerSubscriptionDeleted(subscription) {
  ...
  try {
    ...
    const emailsSent = transferred.filter((c) => c.email_sent).length;
    const emailsFailed = transferred.length - emailsSent;

    console.log(
      `[resellerStripe] Churn complete: reseller=%s transferred=%d emails_sent=%d emails_failed=%d`,
      tenantId,
      transferred.length,
      emailsSent,
      emailsFailed
    ...
    // Per-customer breadcrumb for debugging (especially failed emails,
    // which need manual outreach). Done as separate console.log calls so
    // they're grep-able in Render logs.
