 ...
 * Shows the customer their account info + the plan they're being moved to,
 * then routes them through Stripe Checkout to set up direct billing.
 *
 * No auth required — the grace token IS the credential. Backend validates it
 * on every request (GET to load page data, POST to create checkout).
 *
    ...
    return (
      <PageShell>
        <ErrorCard
          ...
          message="Your 30-day grace period has ended. To restore service, please contact our support team and we'll get you back up and running."
          ctaLabel="Email support@aifrontdeskhelper.com"
    ...
    return (
      <PageShell>
        <ErrorCard
          ...
          message={error || "This link is invalid or has already been used. If you've already set up direct billing, you can log in to your account directly."}
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-stone-200 p-8">
        <div className="text-center mb-6">
          ...
          <p className="text-sm text-stone-500 max-w-md mx-auto leading-relaxed">
            <strong className="text-stone-700">{originating_reseller_name}</strong>{" "}
            ended their partnership with AI Front Desk Helper.
            To keep your service running without interruption, set up direct billing below.
          </p>
        </div>
        ...
        <p className="text-xs text-stone-400 text-center mt-4 leading-relaxed">
          Powered by Stripe. No setup fee. Cancel anytime.<br />
          Your service continues uninterrupted as long as billing is active.
        </p>
      </div>
...
function Row({ label, value }) {
  return (
    <div className="flex items-center justify-between text-sm">
      ...
      <span className="font-semibold text-stone-900 text-right ml-4 truncate max-w-[60%]">{value}</span>
    </div>
  );
}

function ErrorCard({ title, message, ctaLabel, ctaHref }) {
  return (
