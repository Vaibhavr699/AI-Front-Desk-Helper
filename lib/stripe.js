...
async function createCheckoutSession(tenantId, planId, returnUrl) {
    ...
    if (tenant?.business_type === "location") {
        // Standardize branch setup to $197 (currently the Basic setup price ID)
        targetSetupFeeId = "price_1T9PL4G4V3F53niETsVTW4hG";
...
async function createAddonNumberCheckoutSession(tenantId, returnUrl) {
    ...
    const session = await stripe.checkout.sessions.create({
        ...
        line_items: [
            {
                price: "price_1THM11G4V3F53niEsbKk3ruQ", // Correct Price ID for $12/mo add-on
                quantity: 1,
            },
        ],
        metadata: {
