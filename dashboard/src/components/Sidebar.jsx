...
function getNavItems() {
  const user = getUser();
  // Super admin without a tenant — show only admin console
  if (user?.is_super_admin && !user.tenant_id) {
    return [{ to: "/admin", label: "Admin Console", icon: AdminIcon }];
  }
  const items = [...baseNavItems];
  if (user?.is_super_admin) {
    items.push({ to: "/admin", label: "Admin Console", icon: AdminIcon });
  }
  return items;
}
