...
function Avatar({ name, phone }) {
  ...
  const colors = ['bg-blue-100 text-blue-600', 'bg-violet-100 text-violet-600',
        ...
        <div className="flex gap-1 mb-4 overflow-x-auto pb-1">
          {FILTER_TABS.map(tab => {
            ...
            return (
              ...
              >
                {cfg && (
                  <span className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ background: isActive ? 'white' : cfg.dot }} />
                )}
                {tab}
                {count > 0 && (
                  <span className={`text-[10px] px-1.5 py-0.5 rounded-md font-black ${isActive ? 'bg-white/20' : 'bg-stone-100 text-stone-500'}`}>
                    {count}
                  </span>
                )}
              </button>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-stone-100">
                <thead className="bg-stone-50/80">
                  <tr>
                    <th className="px-5 py-3.5 text-left text-[10px] font-bold text-stone-400 uppercase tracking-widest">Contact</th>
                    <th className="px-5 py-3.5 text-left text-[10px] font-bold text-stone-400 uppercase tracking-widest">Status</th>
                    <th className="px-5 py-3.5 text-left text-[10px] font-bold text-stone-400 uppercase tracking-widest">Est. Value</th>
                    <th className="px-5 py-3.5 text-left text-[10px] font-bold text-stone-400 uppercase tracking-widest">Source</th>
                    <th className="px-5 py-3.5 text-left text-[10px] font-bold text-stone-400 uppercase tracking-widest">Last Activity</th>
                    <th className="px-5 py-3.5 text-left text-[10px] font-bold text-stone-400 uppercase tracking-widest">Project</th>
                      ...
                      >
                        ...
                        <td className="px-5 py-3.5 whitespace-nowrap">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <div className="text-sm font-bold text-stone-900 group-hover:text-brand-600 transition-colors leading-tight">
                                {displayName(lead)}
                              </div>
                        ...
                        {/* Source */}
                        <td className="px-5 py-3.5 whitespace-nowrap">
                          <span className={`px-2 py-1 text-[10px] font-black uppercase tracking-widest rounded-md border ${src.bg}`}>
                            {src.label}
                          </span>
                        </td>
                          ...
                          {days !== null ? (
                            <div className="flex items-center gap-1.5">
                              <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${
                                days > 14 ? 'bg-rose-400' : days > 7 ? 'bg-amber-400' : 'bg-emerald-400'
                              }`} />
              ...
              <div className="px-5 py-3 border-t border-stone-100 bg-stone-50/50 flex items-center justify-between">
                ...
                {activeFilter !== 'All' && (
                  <button onClick={() => setActiveFilter('All')} className="text-xs text-brand-600 font-medium hover:underline">
                    Clear filter
                  </button>
                )}
              </div>
