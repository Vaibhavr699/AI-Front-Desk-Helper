...
// JWT_SECRET signs both dashboard and rep-session tokens (the latter bypasses
// MFA), so a missing/default secret in production means forgeable sessions.
// Fail fast rather than serve with a guessable secret.
if (
  process.env.NODE_ENV === "production" &&
  (!process.env.JWT_SECRET || JWT_SECRET === "change-me-in-production")
) {
  console.error("[FATAL] JWT_SECRET is unset or the insecure default in production. Refusing to start.");
  process.exit(1);
}
