...
//   Five roofing services added: asphalt_shingle, metal_standing_seam, tile,
//   slate, flat_epdm. All share the same per-square math via computeRoofingBase.
//   Pitch multipliers from Mighty Dog Roofing's pitch calculator (pure
//   Pythagorean geometry). Stories is a floor-count divisor (footprint calc),
...
function validateInputs(serviceSlug, inputs = {}) {
  ...
  if (serviceSlug === "interior") {
    ...
    // Phase 7 Wave 1 — Roofing (Migration 062, May 11 2026)
    if (!Number.isFinite(inputs.home_floor_sqft) || inputs.home_floor_sqft < 500) {
      errors.push("home_floor_sqft must be >= 500");
    }
...
async function checkSpecialized(verticalId, serviceSlug, inputs) {
  ...
  // Phase 7 Wave 1 — Roofing specialized routing (May 11, 2026)
...
// so scope toggles compound on customer project answers. Disabled toggles
// are skipped (they appear as "quoted separately" in the includes_text
// overlay, which is built separately in routes/estimator.js).
//
// THE V1.1 FIX: prior to today, this function expected modifier_type values
// ('percentage', 'flat_per_project', 'flat_per_item') that the DB CHECK
// constraint disallows. Every scope option was silently being skipped,
// platform-wide, for both painting (since May 8) and roofing (since today
// before this fix). The constraint enforces these enum values:
//
//   'percent'  → integer percent (e.g. 18 means +18%). Unitless multiplier.
//   'flat'     → dollar amount added once when enabled (200 = $200).
//   'per_sq'   → dollar amount per square (100 sqft). Multiplied by
//                context.squares before adding.
//   'per_lf'   → dollar amount per linear foot. Multiplied by
//                context.linear_feet before adding.
//   'per_unit' → dollar amount per item. Multiplied by
//                context.unit_counts[option_key] before adding. If the
//                count isn't supplied (widget hasn't asked yet), the
//                toggle silently skips with a logged reason.
//
// Currency unit reconciliation: vertical_scope_options.price_modifier_default
// is stored in DOLLARS (per scopeOptionsHelper.js documentation and confirmed
// via SELECT on production data, May 11 2026). lib/estimator.js works in
// CENTS everywhere else. So this function multiplies dollars × 100 to get
// cents for flat / per_sq / per_lf / per_unit. 'percent' is unitless so no
// conversion needed.
//
// Context object (V1.1):
//   { squares?, linear_feet?, unit_counts? }
//   Built by calculateRange per service. See the scope-options-context
//   section in calculateRange for which services populate which fields.
// ----------------------------------------------------------------------------
function applyScopeOptions(baseMin, baseMax, scopeOpts, context = {}) {
  ...
  for (const opt of scopeOpts || []) {
    ...
    // ── 'percent' — unitless multiplier. 18 means +18%, factor 1.18. ─────
    if (type === "percent") {
      const factor = 1 + modifier / 100;
      min = Math.round(min * factor);
      max = Math.round(max * factor);
      breakdown.push({ option: opt.option_key, type: "percent", percent: modifier, factor });
      continue;
    }
...
//
// Two-step geometric model based on Mighty Dog Roofing's pitch calculator
// (industry-standard Pythagorean: √(rise² + run²) / run):
//
//   1. Total floor area → footprint:
//        footprint_sqft = home_floor_sqft / floors_count
//   2. Footprint → actual roof surface area via pitch multiplier:
//        roof_sqft = footprint_sqft × pitch_multiplier
//   3. Base cost = (roof_sqft / 100) × per_square_rate
//
// Pitch multipliers are PURE GEOMETRY (no labor adjustment baked in).
//
