      ...
      {isOpen && (
        <div className="absolute left-[2]  top-full mt-2 w-56 border-2 rounded-xl bg-white border border-stone-200 shadow-2xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-1 duration-200">
          <div className="p-1 space-y-1">
            ...
            >
              <div className={`w-8 h-8 rounded-md flex items-center justify-center overflow-hidden border border-stone-200 ${tenantId === parent.id ? "bg-white/20" : "bg-stone-100"}`}>
                {parent.logo_url ? (
            ...
            {children.length > 0 && (
              <>
                ...
                {children.map(child => (
                  ...
                  >
                    <div className={`w-8 h-8 rounded-md flex items-center justify-center overflow-hidden border border-stone-200 ${tenantId === child.id ? "bg-white/20" : "bg-stone-100"}`}>
                       {child.logo_url ? (
                         <img src={child.logo_url} alt={child.name} className="w-full h-full object-cover" />
                       ) : (
                         <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
