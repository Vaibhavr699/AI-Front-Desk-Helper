...
async function createCheckoutSession(tenantId, planId, returnUrl, interval) {
    ...
    // Use annual price ID if annual, otherwise monthly (with override support)
    let monthlyPriceId = isAnnual ? plan.stripePriceIdAnnual : plan.stripePriceId;
    if (hasMonthlyOverride && !isAnnual) {
        // Create an ad-hoc recurring price with the override amount (monthly only)
 ...
 *
 * Apr 19 spec: when a rollup_only HQ adds a self_pays location, the franchisee
 * receives an invite email with a tokenized link. Clicking the link calls
 * routes/franchisee.js which calls THIS function to start the franchisee's
 * own Stripe checkout. The franchisee enters their own payment method and
 * pays for their own plan.
 *
 * Critically different from createCheckoutSession:
 *   - The customer is the CHILD tenant (franchisee), NOT the parent (HQ)
 *   - No setup fee (franchisor already paid HQ tier setup, if any)
 *   - Subscription metadata includes parent_id so post-checkout webhook can
 *     log the relationship and notify the franchisor
 *   - On success, franchisee lands on dashboard via their own login flow
 *     (not the franchisor's dashboard)
 *
 * @param {object} params
 * @param {string} params.childTenantId - Franchisee tenant id (already created in pending state)
 * @param {string} params.parentTenantId - Franchisor HQ tenant id
 * @param {string} params.planId - "basic" | "pro" | "elite" (franchisee's choice)
 * @param {string} params.interval - "monthly" | "annual"
 * @param {string} params.acceptToken - The franchisee_invite_token (round-trips for verification)
 * @param {string} params.successReturnUrl - Where to land after successful checkout
...
async function createFranchiseeInviteCheckoutSession({
    ...
    // Verify child + parent exist and the invite token matches
    ...
    // Create or reuse the franchisee's own Stripe customer (separate from parent)
    ...
    switch (event.type) {
        case "checkout.session.completed": {
            ...
            if (tenantId && type === "addon_number") {
                ...
                // The franchisee just paid for their own subscription. Activate the
                // tenant, mark invite accepted, and (later) notify the franchisor.
                ...
                // Plan subscription completed — update plan + status
                // Elite plan auto-flips brand_mode to white_label (per Apr 19 WL pivot: Elite = WL by default)
                // Pro/Basic never auto-flip (respects any superadmin-granted WL override)
        ...
        case "customer.subscription.updated": {
            ...
            if (tenantId) {
                ...
                if (planId) {
                    ...
                    // Elite upgrade auto-flips brand_mode to white_label (one-way only per Apr 19 pivot).
                    // Downgrades from Elite do NOT auto-flip back — preserves any superadmin-granted WL
                    // for resellers/franchise brands on non-Elite plans.
                    if (planId === "elite") {
