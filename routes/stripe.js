 ...
 * Body: { tenant_id, return_url? }
 *
 * Apr 29, 2026 — Phase 6 Franchise. Used by the franchise paywall page.
 * Refuses if zee is on manual billing or already subscribed.
 */
router.post("/franchise-checkout", async (req, res) => {
    try {
        const tenant_id = getGuaranteedTenantId(req);
        const { return_url } = req.body || {};
        if (!tenant_id) return res.status(400).json({ error: "tenant_id required" });

        const { createFranchiseZeeCheckoutSession } = require("../lib/stripe");
        const result = await createFranchiseZeeCheckoutSession(tenant_id, return_url);
        res.json(result);
    } catch (e) {
        console.error("[Stripe] Franchise checkout error:", e.message);
        res.status(400).json({ error: e.message });
    }
});
