...
import {
  ...
  Shield, User, History, Wrench, Phone, Pencil,
} from "lucide-react";
...
// ─── Technicians Tab ───────────────────────────────────────────────────────
//
// Phase 8B (May 20, 2026). Technicians are people who perform estimate
// visits. They are NOT dashboard login accounts — no password, no invite
// email, no role. Just a contact record: name, email, phone. The phone is
// what the pre-visit briefing SMS routes to when the technician is assigned
// to a booking from the Leads page.
function TechniciansTab() {
  ...
  useEffect(() => { fetchTechs(); }, [activeTenantId]);

  async function fetchTechs() {
    setLoading(true);
    setError("");
    try {
      const tid = activeTenantId && activeTenantId !== "all" ? activeTenantId : null;
      const res = await getTechnicians(tid);
  ...
  async function handleDelete(tech) {
    if (!window.confirm(
      `Remove ${tech.name}? They will be unassigned from any bookings. This can't be undone.`
    )) return;
    try {
  ...
  if (loading && techs.length === 0) {
    return <div className="flex items-center justify-center py-16"><LumaSpin /></div>;
  }
    ...
    <div>
      <div className="flex items-center justify-between mb-4 gap-4">
        <p className="text-sm text-stone-500">
          Estimators who perform visits. Add a cell phone so they can receive a
          pre-visit briefing when assigned to a job.
        </p>
        <button
          onClick={openCreate}
          className="flex items-center gap-2 px-4 py-2 bg-stone-900 text-white text-sm font-bold rounded-xl hover:bg-stone-800 shadow-sm hover:shadow-md transition-all flex-shrink-0"
        >
          ...
          <table className="w-full text-left text-sm text-stone-600">
            <thead className="bg-stone-50 border-b border-stone-200 text-xs uppercase font-semibold text-stone-500">
              <tr>
                ...
                <th className="px-6 py-4">Cell Phone</th>
              ...
              {techs.length === 0 ? (
                <tr>
                  <td colSpan="4" className="px-6 py-10 text-center text-stone-400">
                    No technicians yet. Add your first estimator to enable
                    pre-visit briefing routing.
                  </td>
                </tr>
              ) : (
                techs.map(tech => (
                  <tr key={tech.id} className="hover:bg-stone-50 transition-colors">
                    <td className="px-6 py-4 font-medium text-stone-900">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-teal-100 flex flex-shrink-0 items-center justify-center text-teal-600">
                    ...
                    <td className="px-6 py-4 text-stone-600">
                      {tech.email || <span className="text-stone-300 italic">—</span>}
                    </td>
                    <td className="px-6 py-4">
                      {tech.phone ? (
                        <span className="flex items-center gap-1.5 font-mono text-stone-700">
                        ...
                        <span className="flex items-center gap-1.5 text-amber-600 text-xs font-medium">
                          ...
                          No phone — can't receive briefings
                        </span>
                      )}
                    </td>
            ...
            <div className="p-6">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  ...
                  <p className="text-sm text-stone-500">
                    No login is created — this is just a contact record.
                  </p>
                </div>
              </div>
              ...
              <form onSubmit={handleSave} className="space-y-4">
                ...
                <div className="space-y-1.5">
                  <label className="text-sm font-medium text-stone-700">
                    Cell Phone <span className="text-stone-400 font-normal">(recommended)</span>
                  </label>
                  <input
                    type="tel"
                    value={formPhone}
                  ...
                  <p className="text-xs text-stone-400">
                    Pre-visit briefing texts go here when this technician is
                    assigned to a job.
                  </p>
                </div>
                ...
                <div className="pt-4 flex gap-3">
                  ...
                  >
                    {saving ? <LumaSpin className="w-4 h-4 border-white" /> : (editingId ? "Save Changes" : "Add Technician")}
                  </button>
                </div>
              </form>
            </div>
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="flex gap-1 mb-6 border-b border-stone-200">
        {[
          ...
          { key: "technicians", label: "Technicians",      icon: <Wrench size={15} /> },
