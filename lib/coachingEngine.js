...
function buildScoringPrompt(dimensions) {
  const dims = Array.isArray(dimensions) && dimensions.length > 0 ? dimensions : SCORING_DIMENSIONS;
  const n = dims.length;
  const dimLines = dims
    .map((d, i) => `${i + 1}. ${d.key} — ${d.criteria || d.label}`)
    .join("\n");
  const firstKey = dims[0]?.key || "rapport";
  return `You are an expert home services sales coach analyzing a recorded sales conversation. Score the rep's performance across ${n} dimensions on a 0-10 scale:
...
All ${n} dimensions required in the array, using exactly these dimension keys: ${dims.map((d) => d.key).join(", ")}.`;
}

const SCORING_SYSTEM_PROMPT = buildScoringPrompt(SCORING_DIMENSIONS);
...
async function scoreConversation({ conversationId, force = false } = {}) {
  try {
    ...
    const tenantDimensions = await scorecards.getScoringDimensions(conv.tenant_id);
    const scoringPrompt = buildScoringPrompt(tenantDimensions);
  ...
  : ""}Industry: ${conv.industry || "unknown"}\nSource: ${conv.source_type}\n\nTranscript:\n${transcriptText}`,
    ...
    const validDims = new Set(tenantDimensions.map((d) => d.key));
