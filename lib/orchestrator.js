...
// ═══════════════════════════════════════════════════════════════════════════
// Service area boundary rules — Migration 068 (May 14, 2026)
//
// Tenant-specific: prompt content varies based on tenant.service_area shape.
// Returns an array of rule lines (matches the other *_RULES pattern) so it
// composes cleanly into buildInstructions via .concat(...). Returns an EMPTY
// array when no service area is configured, so the prompt is unchanged for
// tenants who haven't set one — zero behavior drift.
//
// Triggered by the May 13 Mitu Bansal transcript where the AI committed to
// work in Massachusetts despite the business being Omaha-only. Captures
// out-of-area requests as inquiries so the owner can decide to expand.
// ═══════════════════════════════════════════════════════════════════════════
function buildServiceAreaRules(tenant) {
  ...
  if (sa.type === "states") {
    ...
    lines.push(`If the caller's project is in any OTHER state, you MUST politely decline and capture them as an out-of-area inquiry. Example: "I appreciate you reaching out — unfortunately we only work in ${states} right now. Let me grab your info though, in case we expand. Can I get your name, email, and zip code?" Then call capture_lead_info with everything they share. Do NOT proceed with booking. Do NOT promise a future expansion date.`);
    ...
    lines.push(`If the project is clearly outside ${miles} miles (a different metro, a town you'd never drive to from ${homeLabel}), politely decline and capture as out-of-area: "I appreciate you reaching out — unfortunately that's outside our service area. Let me grab your info though, in case we expand. Can I get your name, email, and zip?" Then call capture_lead_info.`);
    lines.push(`If you're genuinely unsure, ASK: "Just so I know — is that within about ${miles} miles of ${homeLabel}?" Trust their answer.`);
    lines.push(`Do NOT proceed with booking for out-of-area callers. Do NOT promise a future expansion date.`);
    ...
    if (sa.home_city) lines.push(`Home base: ${homeLabel}.`);
    lines.push(`When the caller gives a project address, check the ZIP against the list. If it's NOT on the list, politely decline and capture as out-of-area: "I appreciate you reaching out — unfortunately we don't service that ZIP right now. Let me grab your info though. Can I get your name and email?" Then call capture_lead_info.`);
    lines.push(`If the caller mentions a city but not a ZIP, ASK for the ZIP before deciding: "What ZIP code is the property in?" Then check against the list.`);
    lines.push(`Do NOT proceed with booking for out-of-zip callers. Do NOT promise a future expansion date.`);
  }

  lines.push(`STRICT RULES: Never invent service area coverage. Never commit to a project outside your stated boundary. Never dodge the boundary with "let me check with the team" — the boundary is firm. If they push back, hold the line warmly: "I totally understand, but unfortunately that's outside where our crews can work right now." Then capture the lead as out-of-area and move on.`);
...
function buildInstructions(context) {
  ...
  //
  // Applies to inbound AND recovery flows. buildServiceAreaRules returns []
  // when no boundary is configured, so this is a no-op for tenants who
  // haven't set one. The function reads tenant.service_area directly from
  // the DB — independent of the dashboard UI hydration path.
  // ─────────────────────────────────────────────────────────────────────────
