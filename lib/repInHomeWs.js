...
const { repSeatCanRecord } = require("./requireRep");
...
function createInHomeWss() {
  ...
  wss.on("connection", async (ws) => {
    ...
    const [walkthrough, cueEmphasis] = await Promise.all([
      scorecards.getWalkthrough(ctx.tenant_id),
      scorecards.getCueEmphasis(ctx.tenant_id),
    ]);
    send(ws, { type: "walkthrough_config", walkthrough });
  ...
  async function routeUpgrade(req, socket, head, url) {
    ...
    try {
      const r = await db.query(
        `SELECT s.id, s.tenant_id, s.user_id, s.started_at, s.ended_at,
                u.seat_type
    ...
    if (!repSeatCanRecord(session.seat_type)) {
      rejectSocket(socket, 403, "Manager seats cannot record");
      return;
    }
    if (session.ended_at) {
