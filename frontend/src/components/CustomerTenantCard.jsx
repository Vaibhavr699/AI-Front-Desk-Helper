...
export default function CustomerTenantCard({
  ...
  return (
    <div className={`rounded-2xl border p-5 transition-all duration-200 ${styles.card}`}>
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <span className="relative flex w-2 h-2">
            <span className={`absolute inline-flex h-full w-full rounded-full ${styles.dotRing} animate-ping opacity-75`} />
            <span className={`relative inline-flex w-2 h-2 rounded-full ${styles.dot}`} />
          </span>
          <span className={`text-[10px] font-bold uppercase tracking-wider ${styles.labelClass}`}>
            {styles.label}
          </span>
        </div>
      ...
      <h3 className="text-lg font-bold text-stone-900 leading-tight mb-1 truncate" title={customer.name}>
      ...
      {state === "active" && (
        <div className="flex items-center gap-3 mb-5 px-1">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs font-semibold text-stone-900">{metrics.calls}</span>
            <span className="text-[10px] font-semibold text-stone-400 uppercase tracking-wider">calls</span>
          </div>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs font-semibold text-stone-900">{metrics.bookings}</span>
            <span className="text-[10px] font-semibold text-stone-400 uppercase tracking-wider">bookings</span>
          </div>
      ...
      {state === "pending_setup" && (
        <div className="rounded-lg bg-white border border-amber-200 px-3 py-2.5 mb-5 text-xs">
          ...
          <div className="text-stone-400 mt-1">Customer hasn't set their password yet.</div>
        </div>
      )}
      ...
      <div className="flex items-center gap-2">
        {state === "active" && (
          <>
            <button type="button" onClick={() => onEdit?.(customer)} className="flex-1 px-3 py-2 text-xs font-bold text-stone-700 bg-stone-100 hover:bg-stone-200 rounded-lg transition-colors">
              Edit
            </button>
            <button type="button" onClick={() => onRemove?.(customer)} className="flex-1 px-3 py-2 text-xs font-bold text-red-600 bg-red-50 hover:bg-red-100 rounded-lg transition-colors">
              Remove
            </button>
          </>
        )}
        ...
        {state === "pending_setup" && (
          <>
            <button type="button" onClick={() => onResendInvite?.(customer)} className="flex-1 px-3 py-2 text-xs font-bold text-amber-700 bg-amber-100 hover:bg-amber-200 rounded-lg transition-colors">
              Resend welcome
            </button>
            <button type="button" onClick={() => onRemove?.(customer)} className="flex-1 px-3 py-2 text-xs font-bold text-stone-600 bg-stone-100 hover:bg-stone-200 rounded-lg transition-colors">
              Cancel
            </button>
          </>
        )}
        ...
        {state === "removed" && (
          <button type="button" disabled className="flex-1 px-3 py-2 text-xs font-bold text-stone-400 bg-stone-100 rounded-lg cursor-not-allowed">
            Awaiting deletion
          </button>
        )}
      </div>
