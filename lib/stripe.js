    ...
    switch (event.type) {
        ...
        case "customer.subscription.deleted": {
            ...
            if (tenantId) {
                ...
                } else if (subType === "rep_coach") {
                    // AI Rep Coach seat subscription ended. Clear its id only —
                    // never touch the main plan's subscription_status / id.
                    await db.query(
                        "UPDATE tenants SET rep_coach_subscription_id = NULL, updated_at = now() WHERE id = $1",
                        [tenantId]
                    );
                    console.log("[Stripe] Rep Coach subscription canceled tenantId=%s", tenantId);
                } else {
