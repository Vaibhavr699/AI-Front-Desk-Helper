...
// Apr 21, 2026 — Step 9: churn transfer fully wired
// Apr 27, 2026 — Reseller pricing rebuild: checkout now adds METERED prices
//                (voice + SMS overage) to the same subscription as the flat
//                price, so cron meter events have subscription_items to
//                attach charges to. Without this, meter events are silently
//                ignored by Stripe.
// ============================================================================
...
//
// Depends on: migration 037 + 038 + 044, lib/db.js, lib/resellerPlans.js,
...
//
// Apr 27, 2026 — IMPORTANT CHANGE: subscription now includes 3 line items:
//   1. Flat price (monthly or annual flat fee, quantity 1)
//   2. Voice metered price (no quantity — usage-based)
//   3. SMS metered price   (no quantity — usage-based)
//
// All three live on ONE subscription so cron/reportResellerUsage.js can
// post meter events that Stripe automatically attaches to the right
// subscription_item. Without the metered prices on the subscription, meter
// events are accepted by Stripe but never billed.
// ===========================================================================
async function createResellerCheckoutSession({
  ...
  const flatPriceId  = getResellerStripePriceId(tier, interval);
  const voicePriceId = getResellerMeteredPriceId(tier, 'voice');
  const smsPriceId   = getResellerMeteredPriceId(tier, 'sms');
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    // Flat price requires quantity. Metered prices must NOT specify
    // quantity — Stripe rejects the request if quantity is set on a
    // usage-based price.
    line_items: [
...
async function handleResellerSubscriptionUpdated(subscription) {
  ...
  // Apr 27, 2026: subscriptions now have 3 items (flat + voice + sms).
  // We tier-detect off the FLAT price specifically — find the line item
  // whose price ID matches one of the flat env vars.
  const items = subscription.items?.data || [];
  let newTier = null;
  for (const item of items) {
    const t = resolveTierFromPriceId(item.price?.id);
    if (t) { newTier = t; break; }
  }
